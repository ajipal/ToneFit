"""
train_farl_color.py — ToneFit ML Project
=========================================
Model A+: FaRL + Color Feature Fusion Classifier
...
Usage:
    python train_farl_color.py

Outputs:
    models/farl_color_model.pth        — best model checkpoint (by val accuracy)
    results/farl_color_history.json    — per-epoch loss/accuracy history
    results/farl_color_training.png    — training curve plot
    results/farl_color_test_report.txt — classification report on test set
"""
...
MODEL_OUT     = "models/farl_color_model.pth"
HISTORY_OUT   = "results/farl_color_history.json"
PLOT_OUT      = "results/farl_color_training.png"
REPORT_OUT    = "results/farl_color_test_report.txt"
...
def build_classifier_head(feature_dim: int) -> nn.Sequential:
    ...
    Input dim = deep feature dim + COLOR_DIM (color features concatenated upstream).
...
class FaRLModel(nn.Module):
    ...
    def __init__(self, visual_backbone, feature_dim: int = FARL_FEATURE_DIM,
        ...
        self.head = build_classifier_head(feature_dim + color_dim)

    def forward(self, x, color_feats):
        features = self.backbone(x)                           # (B, 512)
        combined = torch.cat([features, color_feats], dim=1) # (B, 512+COLOR_DIM)
...
class ResNeXtFallbackModel(nn.Module):
    ...
    Color features are concatenated with deep features before the head.
    The backbone is frozen; only the head is trained.
    """
    ...
    def forward(self, x, color_feats):
        ...
        combined = torch.cat([features, color_feats], dim=1)
        return self.head(combined)
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for images, color_feats, labels in loader:
        ...
        optimizer.zero_grad()
        outputs = model(images, color_feats)
...
def plot_training_curves(history: dict, best_epoch: int, save_path: str):
    ...
    fig.suptitle("FaRL + Color Features Training Curves", fontsize=14, fontweight="bold")
...
def main():
    ...
    _train_folder = ImageFolder(root=TRAIN_DIR, transform=train_transform)
    _test_folder  = ImageFolder(root=TEST_DIR,  transform=val_transform)
    ...
    test_counts  = Counter(label for _, label in _test_folder.samples)
